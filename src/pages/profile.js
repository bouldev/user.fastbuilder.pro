...
let profilePage = {
	...
	oninit: async () => {
		...
		profilePage.is_can_show_blc = generalInfo.is_can_show_blc;
		profilePage.is_hidden_by_default_blc = generalInfo.is_hidden_by_default_blc;
		profilePage.is_dotcs = generalInfo.is_dotcs;
	...
	view: (vnode) => {
		...
		return m(frame.frame, { pageName: "用户信息", pageIcon },
			...
			profilePage.is_dotcs ? null : m(frame.section, { title: "Points" },
				m("p", "100 pts = 1 CNY"),
			...
			profilePage.is_dotcs ? null : m(frame.section, { title: "月额 Plan" },
			...
			function () {
				if (!profilePage.is_dotcs) {
					return m(frame.section, { title: "fbtoken" },
				...
				!profilePage.helperloaded ? m("p", "请稍候，正在加载") :
					[
						...
						m(frame.sectionGeneralText, [
							...
							m("b", "机器人用户"),
							"。",
							profilePage.is_dotcs ? "辅助用户之创建是 DotCS 正常工作的必要条件。" : "辅助用户之创建是 PhoenixBuilder 正常工作的必要条件。",
			...
			profilePage.is_dotcs ? null : m(frame.section, { title: "fbtoken" },
				m(frame.sectionGeneralText, "fbtoken 是 PhoenixBuilder 用于登入至您的 FastBuilder 用户中心账户的凭证。"),
				m(frame.sectionGeneralText, "点击下方按钮获取。"),
				m(frame.button, {
					onclick: (e) => {
			...
			profilePage.is_dotcs ? null : profilePage.no_security ? null :
			...
			profilePage.is_dotcs ? null : m(frame.section, { title: "密码" },
			...
			profilePage.is_dotcs ? null : profilePage.is_can_show_blc ? null : m(frame.section, { title: "机器人链接口令" },
				m(frame.sectionGeneralText, "您可以使用以下链接口令来链接此账号到机器人。请注意：这将给予其对您账号完整的控制权！"),
				m("p", profilePage.is_hidden_by_default_blc ? (profilePage.bot_linkCode_displayOrdered ? profilePage.bot_linkCode : m("b", "***")) : profilePage.bot_linkCode), //profilePage.is_hidden_by_default_blc
				profilePage.is_hidden_by_default_blc ? null : m(frame.button, {
					onclick: (e) => {
			...
			profilePage.is_dotcs ? null : m(frame.section, { title: "导出账号信息" },
			...
			m(frame.section, { title: "付款记录" },
				!profilePage.is_dotcs ?
					...
					m("p", "请在 DotCS 用户中心 中获取付款记录,")
			),
			profilePage.dotcs_username ? null : m(frame.section, { title: "DotCS账户转移" },
				m(frame.sectionGeneralText, "账户转移前请阅读用户协议")
			),
			profilePage.is_dotcs ? null : profilePage.no_security ? null :
